...
const { publishTalk, updateTalk, deleteTalkById, togglePublic,
    toggleTop, revertTalk, blogGetTalkList, talkLike, cancelTalkLike, getTalkList, getTalkById
} = require("../service/talk/talk.service")
...
class TalkController {
     ...
     * 发布说说
     * */
    async pulishTalk(ctx) {
        try {
            let res = await publishTalk(ctx.request.body)
            ctx.body = result("发布说说成功", { id: res.id })
        } catch (err) {
            console.error(err)
    ...
    async cancelTalkLike(ctx) {
        try {
            ...
            console.error(err)
            return ctx.app.emit("error", throwError(errorCode, "取消点赞事故v啊u"), ctx)
        }
    }
    ...
    async toggleTop(ctx) {
        ...
        let message = Number(is_top) == 1 ? "置顶" : "不置顶"
        try {
    ...
    async getTalkList(ctx) {
        try {
            ...
            console.error(err)
            return ctx.app.emit("error", throwError(errorCode, "获取说说列表失败"), ctx)
        }
    }
    ...
    async getTalkById(ctx) {
        try {
            ...
            console.error(err)
            return ctx.app.emit("error", throwError(errorCode, "获取说说详情失败"), ctx)
        }
    }
