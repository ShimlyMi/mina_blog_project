...
const {
    createComment, cancelThumbUp, deleteComment, replyComment, backGetCommentList, frontGetChildrenComment, frontGetParentComment, getCommentTotal, thumbUpComment
} = require("../service/comment.service")
const { addNotify } = require("./notify.controller")
const { getCurrentTypeName } = require("../utils/tools")

class CommentController {
    /** 新增评论 */
    async addComment(ctx) {
        try {
            let res = await createComment(ctx.request.body)
    ...
    async replyComment(ctx) {
        try {
            let res = await replyComment(ctx.request.body)
            // from_id表示当前大陆人id 发表评论的人和当前登录人不一样才进行消息提示 author_id表示当前被评论的作者的id
            let { type, for_id, from_name, from_id, content, to_id } = ctx.request.body
            // 不是作者自己评论的才给作者消息提示
            if (from_id != to_id) {
                await addNotify({
                    user_id: to_id,
                    type: type,
                    to_id: for_id,
                    message: `您的${getCurrentTypeName(type)}收到了来自于 ${from_name} 的评论回复: ${content}!`
                })
            }
            ctx.body = result("回复评论成功", { res })
        } catch (err) {
            console.error(err)
