...
const { addLike, cancelLike, getIsLikeByIdAndType } = require("../service/like.service");
...
class LikeController {
    ...
    async addLike(ctx) {
        try {
            ...
            if (!for_id) {
                return ctx.app.emit("error", throwError(errorCode, "点赞对象不能为空"))
            }
            if (!type) {
                return ctx.app.emit("error", throwError(errorCode, "点赞类型不能为空"))
            }
            let res
            if (!user_id) {
                res = false
    ...
    async cancelLike(ctx) {
        try {
            const { for_id, type, user_id } = ctx.request.body;
            if (!for_id) {
                return ctx.app.emit("error", throwError(errorCode, "取消点赞对象不能为空"))
            }
            if (!type) {
                return  ctx.app.emit("error", throwError(errorCode, "取消点赞类型不能为空"))
            }
            let res
            if (!user_id) {
                res = false
    ...
    async getIsLikeByIdAndType(ctx) {
        try {
            ...
            ctx.body = result("获取用户是否点赞成功", ctx)
        } catch (err) {
            console.error(err)
