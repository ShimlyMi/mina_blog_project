...
/**
 * 全局类型生命，无需引入，直接再 `.vue` `.ts` `tsx` 文件使用即可获得类型提示
 */
declare global {
  /**
   ...
   * `src/router` 文件里的类型声明
   */
  interface toRouterType extends RouteLocationNormalized {
    meta: {
      role: number,
      keepAlive?: boolean,
      dynamicLevel?: string;
   ...
   */
  interface RouterChildrenConfigsTable {
    /** 子路由地址 `必填` */
    path: string;
    /** 路由名字（对应不要重复，和当前组件的`name`保持一致） `可选` */
    name?: string;
    /** 路由重定向 `可选` */
    redirect?: string;
    /** 按需加载组件 `可选` */
    ...
    meta?: {
      ...
      /** 菜单名臣右侧的额外图标 支持`fontwesome` `iconfont` `element-plus-icon` `可选` */
      extraIcon?: {
        svg?: string;
        name?: string;
      };
      ...
      /**
       * 页面加载动画
       * (有两种形式，一种直接采用vue内置的`transitions`动画，另一种是使用`animate.css`写进、离场动画)
       * `可选`
       */
      transition?: {
        /**
  ...
  interface RouteConfigsTable {
    ...
    meta?: {
      ...
      /** 是否再菜单中提示 (默认`true`) `可选` */
      showLink?: boolean;
      /** 菜单升序排序， 值越高排越后 (只针对顶级路由) `可选` */
      rank?: number;
    };
    /** 子路由配置 */
    children?: Array<RouterChildrenConfigsTable>;
  }
